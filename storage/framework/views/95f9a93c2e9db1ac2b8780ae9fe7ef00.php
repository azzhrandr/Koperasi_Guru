<?php $__env->startSection('title', 'Dashboard Saya - Koperasi Guru'); ?>
        ...
        <div class="col-12 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 p-4 text-white h-100" style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%);">
                ...
                <h2 class="fw-bold mb-4">
                    Rp <?php echo e(number_format($totalSavings, 0, ',', '.')); ?>
                    ...
                    <?php if($activeLoan): ?>
                        ...
                        <div class="row g-2 small text-muted">
                            <div class="col-6">
                                ...
                                <strong class="text-navy fs-5">Rp <?php echo e(number_format($totalBulanan * $activeLoan->sisa_angsuran_bulan, 0, ',', '.')); ?></strong>
                            </div>
                            <div class="col-6 text-end">
                                <span class="d-block">Angsuran Bulanan:</span>
                                <strong class="text-dark">Rp <?php echo e(number_format($totalBulanan, 0, ',', '.')); ?></strong>
                            </div>
                            <div class="col-12 mt-3">
                                <span class="d-block mb-1">Masa Sisa Angsuran: <strong><?php echo e($activeLoan->sisa_angsuran_bulan); ?> dari <?php echo e($activeLoan->lama_angsuran_bulan); ?> Bulan</strong></span>
                                <div class="progress rounded-pill" style="height: 8px;">
                                    <?php 
                                        $percentPaid = (($activeLoan->lama_angsuran_bulan - $activeLoan->sisa_angsuran_bulan) / $activeLoan->lama_angsuran_bulan) * 100;
                                    ?>
                                    <div class="progress-bar bg-success" style="width: <?php echo e($percentPaid); ?>%"></div>
                                </div>
                            </div>
                        </div>
                    <?php else: ?>
                        <div class="text-center py-4 text-muted small">
                            ...
                            <?php if($latestLoan && $latestLoan->status === 'menunggu'): ?>
            ...
            <table class="table custom-table align-middle mb-0 small text-start">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $recentTransactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tx): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr>
                            <td><?php echo e(date('d-m-Y', strtotime($tx->date))); ?></td>
                            <td>
                                <?php if($tx->type === 'simpanan'): ?>
                            ...
                            <td class="text-muted"><?php echo e($tx->label); ?></td>
                            <td class="fw-semibold text-dark">Rp <?php echo e(number_format($tx->amount, 0, ',', '.')); ?></td>
                            <td>
                                <?php if($tx->status === 'sukses' || $tx->status === 'berjalan' || $tx->status === 'lunas'): ?>
                                    <span class="badge bg-success-subtle text-success text-xsmall px-2 py-0.5 rounded">Sukses</span>
                                <?php elseif($tx->status === 'menunggu'): ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Koperasi_SimpanPinjam\resources\views/anggota/dashboard.blade.php ENDPATH**/ ?>