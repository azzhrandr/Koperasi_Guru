...
@section('title', 'Kelola Anggota Guru - Koperasi Guru')
...
<div class="container-fluid p-0">
    ...
    <div class="card border-0 shadow-sm rounded-4 p-3 mb-4">
        <form action="{{ route('admin.anggota.index') }}" method="GET" class="row g-3 align-items-center">
            <div class="col-12 col-md-5">
                <div class="input-group">
                    <span class="input-group-text bg-light border-end-0 text-muted"><i class="fa-solid fa-magnifying-glass"></i></span>
                    <input type="text" class="form-control bg-light border-start-0 ps-0" name="search" value="{{ $search }}" placeholder="Cari berdasarkan nama, NIP, atau email...">
                </div>
            </div>
            ...
            <div class="col-12 col-sm-6 col-md-4 d-flex gap-2">
                ...
                <a href="{{ route('admin.anggota.index') }}" class="btn btn-light border flex-fill"><i class="fa-solid fa-arrows-rotate me-1"></i> Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="table custom-table align-middle mb-0">
                ...
                <tbody>
                    @forelse($anggota as $index => $member)
                        <tr>
                            <td class="ps-4 text-muted">{{ $anggota->firstItem() + $index }}</td>
                            <td>
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        <h6 class="mb-0 text-dark fw-semibold">{{ $member->name }}</h6>
                                        <span class="xsmall text-muted">ID: {{ str_pad($member->id, 4, '0', STR_PAD_LEFT) }}</span>
                                    </div>
                                </div>
                            </td>
                            <td><code class="text-secondary">{{ $member->nip ?? '-' }}</code></td>
                            <td>{{ $member->email }}</td>
                            <td>{{ $member->phone ?? '-' }}</td>
                            <td>
                            ...
                            <td class="pe-4">
                                <div class="d-flex justify-content-center gap-2">
                                    <a href="{{ route('admin.anggota.detail', $member->id) }}" class="btn btn-light btn-sm text-primary" title="Buku Tabungan & Pinjaman">
                                        <i class="fa-solid fa-book-open"></i> Detail
                                    </a>
                                    <button class="btn btn-light btn-sm text-secondary" data-bs-toggle="modal" data-bs-target="#editMemberModal{{ $member->id }}" title="Edit">
                                        <i class="fa-solid fa-pen-to-square"></i> Edit
                                    </button>
                                    <button class="btn btn-light btn-sm text-danger" data-bs-toggle="modal" data-bs-target="#deleteMemberModal{{ $member->id }}" title="Hapus">
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content rounded-4 border-0 shadow">
                                    <div class="modal-header border-bottom-0 pb-0">
                                        <h5 class="modal-title fw-bold text-navy" id="editMemberModalLabel{{ $member->id }}">Edit Biodata Guru</h5>
                                    ...
                                    <form action="{{ route('admin.anggota.update', $member->id) }}" method="POST">
                                        ...
                                        <div class="modal-body py-3">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" name="nip" value="{{ $member->nip }}">
                                            </div>
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-sm">
                                <div class="modal-content rounded-4 border-0 shadow">
                                    <div class="modal-body text-center py-4">
                                        ...
                                        <p class="text-muted small mb-0">Tindakan ini akan menghapus permanen data guru <strong>{{ $member->name }}</strong> beserta seluruh riwayat simpanan/pinjaman.</p>
                                    </div>
