...
@section('title', 'Ledger Simpanan - Koperasi Guru')
...
<div class="container-fluid p-0">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
            <h4 class="fw-bold text-navy mb-1">Detail Simpanan: {{ $user->name }}</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('admin.simpanan.index') }}">Simpanan</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Detail Simpanan</li>
                </ol>
            </nav>
        </div>
        <a href="{{ route('admin.simpanan.index') }}" class="btn btn-light border d-flex align-items-center gap-2">
                    ...
                    @forelse($simpanan as $idx => $s)
                        <tr>
                            ...
                            <td class="text-center">
                                <button class="btn btn-light btn-sm text-danger" data-bs-toggle="modal" data-bs-target="#deleteSavingModal{{ $s->id }}">
                        ...
                        <div class="modal fade" id="deleteSavingModal{{ $s->id }}" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-sm">
                                <div class="modal-content rounded-4 border-0 shadow">
                                    <div class="modal-body text-center py-4">
                                        ...
                                        <p class="text-muted small mb-0">Apakah Anda yakin ingin menghapus transaksi senilai <strong>Rp {{ number_format($s->nominal, 0, ',', '.') }}</strong>Tindakan ini akan mengurangi saldo simpanan anggota secara permanen.</p>
                                    </div>
                                    <div class="modal-footer border-top-0 pt-0 justify-content-center">
                                        <button type="button" class="btn btn-light border px-3" data-bs-dismiss="modal">Batal</button>
                                        <form action="{{ route('admin.simpanan.destroy', $s->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
